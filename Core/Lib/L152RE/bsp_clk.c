...
void  BSP_ClkInit (void)
{
    ...
    RCC_OscInit.OscillatorType      = RCC_OSCILLATORTYPE_HSI;   /* HSI = internal 16MHz internal RC oscillator          */
    RCC_OscInit.HSIState            = RCC_HSI_ON;
    RCC_OscInit.HSICalibrationValue = RCC_HSICALIBRATION_DEFAULT;
    RCC_OscInit.PLL.PLLState        = RCC_PLL_ON;
    RCC_OscInit.PLL.PLLSource       = RCC_PLLSOURCE_HSI;        /* PLLCLK = (HSI * PLLMUL) / PLLDIV = 32MHz (Max)       */
    RCC_OscInit.PLL.PLLMUL          = RCC_PLL_MUL6;
    RCC_OscInit.PLL.PLLDIV          = RCC_PLL_DIV3;
    if (HAL_RCC_OscConfig(&RCC_OscInit) != HAL_OK)
    {
        while(1u);                                              /* STOP if error                                        */
    }

    __HAL_RCC_PWR_CLK_ENABLE();                                 /* Set Voltage scale1 as MCU will run at 32MHz          */
    __HAL_PWR_VOLTAGESCALING_CONFIG(PWR_REGULATOR_VOLTAGE_SCALE1);

                                                                /* Wait until Regulator is ready in the seleceted ...   */
    while (__HAL_PWR_GET_FLAG(PWR_FLAG_VOS) != RESET) {         /* ...voltage range.                                    */
        ;
    }
    ...
    RCC_ClkInit.ClockType     = (RCC_CLOCKTYPE_SYSCLK  |
                                 ...
                                 RCC_CLOCKTYPE_PCLK1   |
                                 RCC_CLOCKTYPE_PCLK2);
    RCC_ClkInit.SYSCLKSource   = RCC_SYSCLKSOURCE_PLLCLK;       /* SYSCLK  = PLLCLK = 32MHz                             */
    RCC_ClkInit.AHBCLKDivider  = RCC_SYSCLK_DIV1;               /* HCLK    = AHBCLK = PLLCLK / AHBPRES(1) = 32MHz.      */
    RCC_ClkInit.APB1CLKDivider = RCC_HCLK_DIV1;                 /* APB1CLK = AHBCLK / APB1DIV(1)          = 32MHz.      */
    RCC_ClkInit.APB2CLKDivider = RCC_HCLK_DIV1;                 /* APB2CLK = AHBCLK / APB2DIV(1)          = 32MHz.      */
